...
class CatsRepository @Inject constructor() {
    ...
    fun getCatById(catId: Long): Flow<Cat?> {
        return getCats().map { cats -> cats.firstOrNull { it.id == catId } }
    }

    fun delete(catId: Long) {
        catsFlow.update { oldList ->
            oldList.filter { it.id != catId }
        }
    }

    fun deleteSelectedCats(multiChoiceState: MultiChoiceState<Cat>) {
        catsFlow.update { oldList ->
            oldList.filter { cat -> multiChoiceState.isChecked(cat).not() }
        }
    }

    fun toggleIsFavorite(catId: Long) {
        catsFlow.update { oldList ->
            oldList.map { cat ->
                if (cat.id == catId) {
